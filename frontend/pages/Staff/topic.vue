                    ...
                    <v-card-text class="bg-white">
                        <v-form @submit.prevent="saveMember">
                            <v-row>
                                ...
                                <v-col cols="12" md="12">
                                    <center>
                                        <v-btn type="submit" color="primary" class="text-center ma-3">{{ form.id_topic ? 'อัปเดต':'บันทึก' }}</v-btn>
                        ...
                        <tr v-for="(items,index) in result" :key="items.id_topic">
                            ...
                            <td class="border text-center">
                                <center>
                                    ...
                                    <v-btn class="text-center text-white ma-3" color="red" @click="del(items.id_topic)">ลบ</v-btn>
                                </center>
                            </td>
                        </tr>
...
const saveMember = async()=>{
    ...
    try {
        form.value.id_topic
        ?await axios.put(`${staff}/topic/update/${form.value.id_topic}`,form.value,{headers:{Authorization:`Bearer ${token}`}})
...
const del = async(id_topic:number)=>{
    ...
    try {
        await axios.delete(`${staff}/topic/del/${id_topic}`,{headers:{Authorization:`Bearer ${token}`}})
        await fetch()
        await reset()
    } catch (error) {
